        ...
        <form @submit.prevent="handleRegister" class="register-form">
          <div class="form-group">
            <label class="form-label">
              <font-awesome-icon icon="mobile" class="input-icon" />
              手机号/邮箱
            </label>
          ...
          <div class="form-agreement">
            <label class="agreement-label">
              ...
              <span>我已阅读并同意<a href="#" class="agreement-link" @click.prevent>《用户协议》</a>和<a href="#" class="agreement-link" @click.prevent>《隐私政策》</a></span>
            </label>
          </div>
...
const sendVerifyCode = () => {
  ...
  setTimeout(() => {
    ...
    startCountdown()
    alert('验证码已发送，请注意查收（演示验证码为：1234）')
  }, 1000)
}
...
const handleRegister = () => {
  ...
  // 演示：验证码校验（实际应该与后端验证）
  if (formData.value.verifyCode !== '1234') {
    errorMessage.value = '验证码错误，请输入正确的验证码'
    return
  }
  ...
  // 注册成功，保存用户信息（模拟）
  localStorage.setItem('registeredAccount', formData.value.account)
  
  successMessage.value = '注册成功！正在跳转到登录页面...'

  // 注册成功后触发事件，通知父组件切换到登录弹窗
  setTimeout(() => {
